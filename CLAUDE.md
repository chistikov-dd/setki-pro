...
- **Offline-only**: Работа только в локальном режиме (LAN)
- **Real-time**: WebSocket синхронизация между столами
- **LAN режим**: Локальный сервер (Axum) обязателен для работы
- **Роли**: Администратор (логин/пароль через setki.pro), Судьи (IP + PIN + имя + номер стола)

### Workflow на турнире

```
1. Подготовка (с интернетом):
   Админ → Вход (логин/пароль с setki.pro) → Автозапуск локального сервера → Выбор турнира → "Скачать турнир" → SQLite кэш

2. На турнире (с/без интернета):
   Админ → Показать IP судьям
   Судья → Вход (ввод IP, PIN, имя, номер стола)

3. После турнира (с интернетом):
   Админ → "Выгрузить результаты" → setki.pro
```

### Режимы работы

- **Local Server**: Админ автоматически запускает Axum сервер при входе (порты 8081-8091)
- **Local Client**: Судьи подключаются к локальному серверу по IP

**ВАЖНО**:
- Онлайн-режим удалён - работа только через локальный сервер
- Админ автоматически становится сервером при авторизации
- Судьи всегда подключаются как клиенты
- Авторизация админа требует временного интернет-подключения для проверки на setki.pro
- Скачивание турнира требует интернет-подключения
- Выгрузка результатов требует интернет-подключения
